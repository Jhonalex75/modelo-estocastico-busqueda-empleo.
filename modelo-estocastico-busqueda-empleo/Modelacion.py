...
class StochasticJobSearchModel:
    ...
    def create_profile_tab(self):
        ...
        ttk.Label(comp_frame, text="Educación (E):", style='Heading.TLabel').grid(row=0, column=0, sticky='w', pady=5)
        education_scale = ttk.Scale(comp_frame, from_=0.0, to=1.0, variable=self.education_score, 
                                   length=300, command=self.update_cqi)
        education_scale.grid(row=0, column=1, padx=10, pady=5)
        education_label = ttk.Label(comp_frame, text="0.89")
        education_label.grid(row=0, column=2, pady=5)
        self.education_label = education_label
        
        ttk.Label(comp_frame, text="Experiencia (X):", style='Heading.TLabel').grid(row=1, column=0, sticky='w', pady=5)
        experience_scale = ttk.Scale(comp_frame, from_=0.0, to=1.0, variable=self.experience_score, 
                                    length=300, command=self.update_cqi)
        experience_scale.grid(row=1, column=1, padx=10, pady=5)
        experience_label = ttk.Label(comp_frame, text="0.92")
        experience_label.grid(row=1, column=2, pady=5)
        self.experience_label = experience_label
        
        ttk.Label(comp_frame, text="Habilidades Técnicas (T):", style='Heading.TLabel').grid(row=2, column=0, sticky='w', pady=5)
        technical_scale = ttk.Scale(comp_frame, from_=0.0, to=1.0, variable=self.technical_score, 
                                   length=300, command=self.update_cqi)
        technical_scale.grid(row=2, column=1, padx=10, pady=5)
        technical_label = ttk.Label(comp_frame, text="0.88")
        technical_label.grid(row=2, column=2, pady=5)
        self.technical_label = technical_label
        
        ttk.Label(comp_frame, text="Logros (A):", style='Heading.TLabel').grid(row=3, column=0, sticky='w', pady=5)
        achievements_scale = ttk.Scale(comp_frame, from_=0.0, to=1.0, variable=self.achievements_score, 
                                     length=300, command=self.update_cqi)
        achievements_scale.grid(row=3, column=1, padx=10, pady=5)
        achievements_label = ttk.Label(comp_frame, text="0.91")
        achievements_label.grid(row=3, column=2, pady=5)
        self.achievements_label = achievements_label
        ...
        market_scale = ttk.Scale(market_frame, from_=0.8, to=1.5, variable=self.market_factor, 
                                 length=300, command=self.update_cqi)
        market_scale.grid(row=0, column=1, padx=10, pady=5)
        market_label = ttk.Label(market_frame, text="1.15")
        market_label.grid(row=0, column=2, pady=5)
        ...
        apps_spinbox = ttk.Spinbox(market_frame, from_=1, to=10, width=10, 
                                  textvariable=self.apps_per_week, command=self.update_cqi)
...
Este modelo matemático riguroso proporciona una base científica para optimizar tu estrategia de búsqueda de empleo, identificando los puntos de mejora más impactantes.
    ...
    def update_cqi(self, *args):
        ...
        self.education_label.config(text=f"{self.education_score.get():.2f}")
        self.experience_label.config(text=f"{self.experience_score.get():.2f}")
        self.technical_label.config(text=f"{self.technical_score.get():.2f}")
        self.achievements_label.config(text=f"{self.achievements_score.get():.2f}")
        self.market_label.config(text=f"{self.market_factor.get():.2f}")
    ...
    def run_monte_carlo(self):
        ...
        try:
            probs = [
                self.s1_success.get() / self.apps_submitted.get(),
                self.s2_success.get() / self.s1_success.get(),
                self.s3_success.get() / self.s2_success.get(),
                self.s4_success.get() / self.s3_success.get(),
                self.s5_success.get() / self.s4_success.get()
            ]
            if any(p < 0 or p > 1 for p in probs):
                raise ValueError("Las probabilidades deben estar entre 0 y 1.")

        except (ValueError, ZeroDivisionError) as e:
            messagebox.showerror("Error en Simulación", f"Verifica que los datos empíricos sean válidos. {e}")
            return
            
        success_times = []
        success_prob = np.prod(probs)
        if success_prob <= 0:
            messagebox.showerror("Error en Simulación", "La probabilidad de éxito es cero. No se puede simular.")
            return

        for _ in range(n_sims):
            aplicaciones_necesarias = np.random.geometric(success_prob)
            tiempo_semanas = aplicaciones_necesarias / apps_per_week
            success_times.append(tiempo_semanas)
        
        # Análisis de resultados de la simulación
        tiempos = np.array(success_times)
        mean_time = np.mean(tiempos)
        median_time = np.median(tiempos)
        ci_low = np.percentile(tiempos, 2.5)
        ci_high = np.percentile(tiempos, 97.5)

        # Mostrar resultados en el Text widget
        results_text = f"Simulación de {n_sims} escenarios de búsqueda de empleo:\n\n"
        results_text += f"- Tiempo medio hasta la oferta: {mean_time:.2f} semanas\n"
        results_text += f"- Mediana de tiempo hasta la oferta: {median_time:.2f} semanas\n"
        results_text += f"- Intervalo de Confianza del 95%: [{ci_low:.2f}, {ci_high:.2f}] semanas\n"
